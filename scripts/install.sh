...
usage() {
  ...
  printf "    --force-reinstall                       Force a full fresh re-onboard (vLLM lifecycle is handled by onboard preflight)\n"
  ...
  printf "    NEMOCLAW_MODEL                          Inference model tag\n"
  printf "    NEMOCLAW_BACKEND_ENDPOINT               Override auto-detected backend URL\n"
  printf "    NEMOCLAW_VLLM_MODEL                     Override Station picker (skips prompt; e.g. Qwen/Qwen2.5-72B-Instruct)\n"
...
select_backend() {
  # 1) Force-reinstall vLLM if requested. The `nemoclaw onboard` preflight
  #    handles the Station vLLM lifecycle (model picker, container start /
  #    replace, HF token, /health wait); the installer just records that
  #    vLLM is the chosen backend and lets the wizard do the work.
  if [[ -n "${FORCE_REINSTALL:-}" ]]; then
    info "Backend: vLLM (force reinstall — wizard will tear down and relaunch)"
  ...
  # 2) Station platform — vLLM is the preferred backend. The container
  #    lifecycle moved to the wizard's [1/8] preflight in src/lib/pre-flight-check.ts;
  #    the installer just records the choice here.
  if [[ "${NEMOCLAW_DETECTED_PLATFORM:-}" == "station" ]] && detect_gpu; then
    info "Backend: Station platform — vLLM (lifecycle managed by onboard preflight)"
    NEMOCLAW_SELECTED_BACKEND="vllm"
    NEMOCLAW_SELECTED_BACKEND_ENDPOINT="http://127.0.0.1:8000"
    write_backend_config
    return 0
...
main() {
  ...
  # Non-Station local inference is opt-in via NEMOCLAW_PROVIDER. Both helpers
  # short-circuit when the env var doesn't match, so the cloud / Station path
  # is unaffected (Station vLLM lifecycle is handled in onboard preflight).
