...
- After changing docs, regenerate the user skills:

  ```bash
  python3 scripts/docs-to-skills.py docs/ .agents/skills/ --prefix nemoclaw-user
  ```

- Include generated `.agents/skills/nemoclaw-user-*` updates in normal docs PRs.
- During release prep, run `nemoclaw-contributor-update-docs`, make doc version bumps, regenerate user skills, then open the docs refresh PR.
